...
namespace GraphProject
{
    public partial class GraphChartRsi : Form
    {
        ...
        private int _lenghtMa = 200; // Is plotted
        private int _lenghtMa2 = 50;
        ...
        private void InitializeAttributes()
        {
            ...
            _tradingAlgo = new MaAlgo();
            _tradeManager = new TradeManager();
            _backtest = new Backtest(100000, 125000);
        }
        ...
        private void CalcAndPlotAlgosAndIndicators(List<DailyDataPoint> listOfData)
        {
            ...
            for (int i = 0; i < listOfData.Count; i++)
            {
                ...
                listOfData[i]._MA = (new MovingAverage(listOfData, i, _lenghtMa)).CalculateMa();
                listOfData[i]._MA2 = (new MovingAverage(listOfData, i, _lenghtMa2)).CalculateMa();
                ...
                if (i >= _lenghtMa)
                {
                    ...
                    chartValues5[i - _lenghtMa].DateTime = TimeTranslation(listOfData[i]._MilliSeconds);
                    chartValues5[i - _lenghtMa].Value = listOfData[i]._MA;
                }

                // Plot algo buy signals in graph and save trade
                if (_tradingAlgo.AlgoBuy(listOfData,i))
                {
                ...
                if (_tradingAlgo.AlgoSell(listOfData, i))
                {
        ...
        private void PortfolioAndBacktest(List<DailyDataPoint> listOfData)
        {
            ...
            for (int i = 0; i < _tradeManager.GetTradeList.Count; i++)
            {
                if (_tradeManager.GetTradeList[i].Finished)
                {
                    ...
                    chartValues2[i + 1].Value = _backtest.ChangePortFolValue(_tradeManager, i);
        ...
        private void BacktestData()
        {
            ...
            lbl_Value_Winners_Procent.Text = string.Format("{0:N1}", _backtest.Winners(_tradeManager));
            ...
            lbl_Value_Profit_Factor.Text = string.Format("{0:N1}", _backtest.ProfitFactor(_tradeManager));
            lbl_Value_CAGR.Text = string.Format("{0:N1}", _backtest.CagrAlternative(_dataList));
            lbl_Value_TimeSpan_Start.Text = _backtest.TimespanStart(_dataList);
            lbl_Value_TimeSpan_Finish.Text = _backtest.TimespanFinish(_dataList);
            lbl_Value_Name_Algo.Text = _tradingAlgo.AlgoName;
            lbl_Value_Sharp_Ratio.Text = string.Format("{0:N2}", _backtest.SharpRatio(_tradeManager, _dataList));
        ...
        // ------------------------- To do list ----------------------------
        // The equity curve needs to be updated at every day during a trade, to capture the real
        // volatility. And there by get the correct sharpratio and max draw down.

        // Import more stockdata from avanza api to sql, maybe trough google docs
        // Adapt the interaface with a list of those stocks and a list of algos.
    }
}
